...
@flow(name="generate_tv_series_sitemaps", log_prints=True)
def generate_tv_series_sitemaps():
    ...
    for i in range(count):
        series = get_sitemap_media_tv_series(config, i)
        sitemap_entries = []
        ...
        config.storage_client.upload(f"tv-series/{i}.xml.gz", gzipped_sitemap)
        logger.info(f"  - Uploaded tv-series/{i}.xml.gz")
