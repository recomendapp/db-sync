...
@flow(name="generate_playlist_sitemaps", log_prints=True)
def generate_playlist_sitemaps():
    ...
    for i in range(count):
        playlists = get_sitemap_playlists(config, i)
        sitemap_entries = []
        ...
        config.storage_client.upload(f"playlists/{i}.xml.gz", gzipped_sitemap)
        logger.info(f"  - Uploaded playlists/{i}.xml.gz")
