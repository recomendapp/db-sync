...
def sync_data(db_client: DBClient, ts_client: TypesenseClient):
    ...
    with db_client.connection() as conn:
        ...
        with conn.cursor('person_sync_cursor') as cursor: # Changed cursor name
            ...
            while True:
                ...
                documents = [Mapper.person(row) for row in rows] # Changed Mapper.movie to Mapper.person
                if not documents:
                    continue
...
@flow(name="sync_typesense_persons", log_prints=True) # Changed flow name
def sync_typesense_persons():
    logger = get_run_logger()
    logger.info("Starting synchronization with Typesense for persons...") # Changed log message
    ...
    logger.info("Successfully synchronized persons.") # Changed log message
