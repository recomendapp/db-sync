...
def sync_data(db_client: DBClient, ts_client: TypesenseClient):
    ...
    with db_client.connection() as conn:
        ...
        with conn.cursor('playlist_sync_cursor') as cursor: # Changed cursor name
            ...
            while True:
                ...
                documents = [Mapper.playlist(row) for row in rows] # Changed Mapper.movie to Mapper.playlist
                if not documents:
                    continue
...
@flow(name="sync_typesense_playlists", log_prints=True) # Changed flow name
def sync_typesense_playlists():
    logger = get_run_logger()
    logger.info("Starting synchronization with Typesense for playlists...") # Changed log message
    ...
    logger.info("Successfully synchronized playlists.") # Changed log message
