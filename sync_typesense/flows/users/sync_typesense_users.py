...
def sync_data(db_client: DBClient, ts_client: TypesenseClient):
    ...
    with db_client.connection() as conn:
        ...
        with conn.cursor('user_sync_cursor') as cursor: # Changed cursor name
            ...
            while True:
                ...
                documents = [Mapper.user(row) for row in rows] # Changed Mapper
                if not documents:
                    continue
...
@flow(name="sync_typesense_users", log_prints=True) # Changed flow name
def sync_typesense_users():
    logger = get_run_logger()
    logger.info("Starting synchronization with Typesense for users...") # Changed log message
    ...
    logger.info("Successfully synchronized users.") # Changed log message